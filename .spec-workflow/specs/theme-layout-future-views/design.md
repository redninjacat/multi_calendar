...
This design restructures the Multi Calendar theming system by introducing three mixins (`MCalEventTileThemeMixin`, `MCalTimeGridThemeMixin`, `MCalAllDayThemeMixin`), renaming the Day View theme class and cascade flag, slimming `MCalThemeData` to 8 global properties, and replacing every hardcoded visual layout value in widget code with a theme property. The mixin-based architecture enables future views to compose only the property groups they need.
...
4. **13 properties move off `MCalThemeData`** into per-view themes via mixins. 9 properties move from `MCalMonthThemeData` into `MCalEventTileThemeMixin`.
5. **~45 hardcoded numeric literals** in widget code are replaced by new theme properties with master defaults that preserve the current appearance.
...
- **`MCalThemeData.fromTheme(ThemeData)`**: Master defaults factory. Modified to populate only 8 global properties plus sub-themes via their `.defaults()` factories.
...
- **`MCalMonthThemeData.defaults(ThemeData)`**: Extended with new layout properties and mixin properties.
...
| `lib/src/styles/mcal_month_theme_data.dart` | Mix in `MCalEventTileThemeMixin`; remove 9 directly-declared properties (now from mixin); add 19 new layout properties (16 + 3 Month-specific resize handle dimensions); update all methods |
...
graph TD
    subgraph Mixins
        ...
        TGM[MCalTimeGridThemeMixin<br/>48 properties]
        ADM[MCalAllDayThemeMixin<br/>17 properties]
    end

    subgraph Current Views
        DVT[MCalDayViewThemeData<br/>+ 6 own properties<br/>= 89 total]
        MT[MCalMonthThemeData<br/>+ 47 own properties<br/>= 65 total]
    end
    ...
    subgraph Parent
        TD[MCalThemeData<br/>8 properties<br/>+ dayViewTheme<br/>+ monthTheme]
    end
...
```
lib/src/styles/
├── mcal_theme.dart                      # MCalThemeData (8 props), MCalTheme InheritedWidget
├── mcal_day_view_theme_data.dart        # MCalDayViewThemeData (89 props) — renamed
├── mcal_month_theme_data.dart           # MCalMonthThemeData (62 props)
├── mcal_event_tile_theme_mixin.dart     # NEW: MCalEventTileThemeMixin (18 abstract getters)
├── mcal_time_grid_theme_mixin.dart      # NEW: MCalTimeGridThemeMixin (48 abstract getters)
└── mcal_all_day_theme_mixin.dart        # NEW: MCalAllDayThemeMixin (17 abstract getters)
```
...
- **Pattern**: Same as above. 37 existing properties moved from `MCalDayThemeData` + 11 new layout properties = 48 total.
...
- **Pattern**: Same as above. 4 properties moved from `MCalThemeData` + 4 from `MCalDayThemeData` + 9 new layout properties = 17 total.
...
class MCalThemeData extends ThemeExtension<MCalThemeData> {
  ...
  // copyWith, lerp, ==, hashCode follow existing patterns with 8 properties
}
```
...
- **Composition**: `MCalEventTileThemeMixin` (18) + `MCalTimeGridThemeMixin` (48) + `MCalAllDayThemeMixin` (17) + 6 own = **89 properties**
- **Constructor**: All 89 properties as named optional parameters
- **`defaults(ThemeData)`**: Populates all 89 properties. New layout property defaults match current hardcoded values.
- **`copyWith`, `lerp`, `==`, `hashCode`**: Include all 89 properties following existing patterns.
...
### Component 6: MCalMonthThemeData (modified)
...
```dart
class MCalMonthThemeData with MCalEventTileThemeMixin {
...
### MCalThemeData — Final Property List (8 properties)
...
| `monthTheme` | `MCalMonthThemeData?` | `MCalMonthThemeData.defaults(theme)` | Retained |
...
| `eventTileBorderWidth` | `double?` | `0.0` (no border by default) | `0.0` (no border by default) | From `MCalMonthThemeData` |
| `eventTileBorderColor` | `Color?` | `null` | `null` | From `MCalMonthThemeData` |
...
| `multiDayEventBackgroundColor` | `Color?` | `colorScheme.primary` at 80% alpha | `colorScheme.primary` at 80% alpha | From `MCalMonthThemeData` |
...
### MCalTimeGridThemeMixin — Property List (48 properties)
...
### MCalAllDayThemeMixin — Property List (17 properties)
...
### MCalMonthThemeData — Own Properties (28 retained + 19 new = 47)
...
### MCalMonthThemeData — Removed Direct Declarations (9, now from mixin)

| Property | Previously On | Now From |
|----------|--------------|----------|
| `dropTargetTileBackgroundColor` | `MCalMonthThemeData` | `MCalEventTileThemeMixin` |
| `dropTargetTileInvalidBackgroundColor` | `MCalMonthThemeData` | `MCalEventTileThemeMixin` |
| `dropTargetTileCornerRadius` | `MCalMonthThemeData` | `MCalEventTileThemeMixin` |
| `dropTargetTileBorderColor` | `MCalMonthThemeData` | `MCalEventTileThemeMixin` |
| `dropTargetTileBorderWidth` | `MCalMonthThemeData` | `MCalEventTileThemeMixin` |
| `resizeHandleColor` | `MCalMonthThemeData` | `MCalEventTileThemeMixin` |
| `eventTileBorderColor` | `MCalMonthThemeData` | `MCalEventTileThemeMixin` |
| `eventTileBorderWidth` | `MCalMonthThemeData` | `MCalEventTileThemeMixin` |
| `multiDayEventBackgroundColor` | `MCalMonthThemeData` | `MCalEventTileThemeMixin` |
...
2. Update `themeColor:` source from `theme.eventTileBackgroundColor` to `theme.dayViewTheme?.eventTileBackgroundColor` (or `theme.monthTheme?.eventTileBackgroundColor`)
...
final tileColor = resolveEventTileColor(
  themeColor: theme.monthTheme?.multiDayEventBackgroundColor,
  eventColor: event.color,
  enableEventColorOverrides: theme.enableEventColorOverrides,
  defaultColor: defaults.monthTheme!.multiDayEventBackgroundColor!,
);
...
| `time_grid_events_layer.dart` | Replace hardcoded margin, font sizes, border width with `timedEventMargin`, `timedEventCompactFontSize`, `timedEventNormalFontSize`, `timedEventKeyboardFocusBorderWidth`, `eventTileBorderWidth`, `eventTileBorderColor` from `dayViewTheme`; update cascade calls |
...
| `all_day_events_section.dart` | Replace 9 hardcoded values with allDay* properties from `dayViewTheme`; update cascade calls |
| `day_cell_widget.dart` | Replace 6 hardcoded values with `dateLabelPadding`, `cellBorderWidth`, `regionContentPadding`, `regionIconSize`, `regionIconGap`, `regionFontSize` from `monthTheme` |
| `week_row_widget.dart` | Replace hardcoded `2.0`/`1.5`/`24` with `keyboardSelectionBorderWidth`, `keyboardHighlightBorderWidth`, `dateLabelCircleSize` from `monthTheme`; update cascade calls |
| `week_number_cell.dart` | Replace hardcoded `36.0`/`0.5` with `weekNumberColumnWidth`, `weekNumberBorderWidth` from `monthTheme` |
| `weekday_header_row_widget.dart` | Replace hardcoded padding with `weekdayHeaderPadding` from `monthTheme` |
| `mcal_month_multi_day_tile.dart` | Replace hardcoded padding/radius with `multiDayTilePadding`, `multiDayTileBorderRadius` from `monthTheme`; update cascade calls |
| `month_page_widget.dart` | Update cascade calls; `dropTargetTileBorderWidth` default is `1.5` (from mixin via `monthTheme`). **Fix:** change `resizeHandleColor` read from `theme.dayTheme` to `theme.monthTheme` (current code incorrectly reads Day View theme in a Month View widget). Replace hardcoded resize handle dimensions (`width: 2`, `height: 16`, `BorderRadius.circular(1)`) with `resizeHandleVisualWidth`, `resizeHandleVerticalMargin` (height = tileHeight − 2 × margin), `resizeHandleBorderRadius` from `monthTheme` |
| `mcal_month_default_week_layout.dart` | Replace hardcoded `2.0`/`2.0` with `weekLayoutDateLabelPadding`, `weekLayoutBaseMargin` accessed via `MCalTheme.of(context).monthTheme` |
...
2. **Consumer provides partial theme with some mixin properties**
   - **Handling**: Non-null properties take precedence. Null properties fall through to `defaults.dayViewTheme!.property!` or `defaults.monthTheme!.property!`. The `!` is safe because `fromTheme()` always creates sub-themes via `.defaults()`.
...
- Verify `copyWith` works with remaining 8 properties.
...
- Verify Month View event tiles continue to use `eventTileBorderWidth`/`eventTileBorderColor` from `monthTheme`.
